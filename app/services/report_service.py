...
def get_project_dashboard_data(complex_name: str, property_type: str = None):
    ...
    houses_in_complex = mysql_session.query(EstateHouse).filter_by(complex_name=complex_name).order_by(EstateHouse.name).all()
    houses_data = []
    ...
    total_deals_volume = mysql_session.query(func.sum(EstateDeal.deal_sum)).join(EstateSell).join(EstateHouse).filter(
        EstateHouse.complex_name == complex_name,
        EstateDeal.deal_status_name.in_(sold_statuses)
    ).scalar() or 0

    total_income = mysql_session.query(func.sum(FinanceOperation.summa)).join(EstateSell).join(EstateHouse).filter(
        EstateHouse.complex_name == complex_name,
        FinanceOperation.status_name == 'Проведено'
    ).scalar() or 0
    ...
    plans_query = planning_session.query(planning_models.SalesPlan).filter_by(complex_name=complex_name, year=today.year)
    if property_type:
    ...
    recent_deals = mysql_session.query(
